...
interface BrowserExecutionWrapperConfig {
    features: {
        evalify: boolean;
        functionify: boolean;
        node_vm: boolean;
...
export type BrowserExecutionWrapperMode = "functionify" | "evalify";

export function resolveBrowserExecutionWrapperMode(features: {
    functionify: boolean;
    evalify: boolean;
    node_vm: boolean;
}): BrowserExecutionWrapperMode | null {
    ...
    if (features.node_vm) {
        throw new Error("features.node_vm is not supported in the browser playground");
    }

    return null;
}

export function wrapProgramWithBrowserExecutionMode(
    ast: object,
    names: NameGenerator,
    ...
    if (
        ...
        throw new Error("features.encryption is not supported in the browser playground");
    }
